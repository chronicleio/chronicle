...
impl Chronicle {
    ...
    pub async fn create_dataset(
        &self,
        dataset: Dataset,
    ) -> Result<Versioned<Dataset>, ChronicleError> {
        Ok(self.catalog.create_dataset(dataset).await?)
    }

    pub async fn open_dataset(&self, name: &str) -> Result<DatasetClient, ChronicleError> {
        let dataset = self.catalog.get_dataset(name).await?;
        let xunit = self.xunit.clone().ok_or_else(|| {
            ChronicleError::Internal(
                "xunit client is required for dataset read/write in the MVP".into(),
            )
        })?;
...
impl DatasetClient {
    ...
    pub async fn write(&self, event: Event) -> Result<Offset, ChronicleError> {
    ...
    ) -> Result<Offset, ChronicleError> {
        self.write(Event::new(payload.into())).await
    }

    pub async fn read_all(&self) -> Result<Vec<Event>, ChronicleError> {
        let response = self
            .xunit
...
async fn next_dataset_offset(
    xunit: &dyn XunitClient,
    dataset: &str,
...
mod tests {
    ...
    async fn dataset_create_write_and_read_round_trip() {
        ...
        let chronicle = Chronicle::with_xunit(catalog, ChronicleOptions::new(), xunit);
        ...
        let dataset = chronicle.open_dataset("events").await.unwrap();
        let offset = dataset
