...
struct InflightBatch {
    max_offset: i64,
    callbacks: Vec<(i64, oneshot::Sender<Result<Offset, LyraError>>)>,
}
...
impl StateMachine {
    ...
    pub async fn record(&self, event: UserEvent) -> Result<Offset, LyraError> {
        ...
        self.inner
            ...
            .await
            .map_err(|_| LyraError::Internal("state machine stopped".into()))?;
        reply_rx
            .await
            .map_err(|_| LyraError::Internal("state machine dropped".into()))?
    }
...
where
    ...
    OpFut: Future<Output = (UnitInfo, Result<R, LyraError>)> + Send + 'static,
{
...
async fn fence_members(state: &State, members: &[UnitInfo]) -> Result<(), LyraError> {
...
async fn recover(state: &mut State) -> Result<(), LyraError> {
    ...
    .await
    .map_err(|e| LyraError::UnitNotEnough(e.to_string()))?;
            ...
            _ = cancel.cancelled() => {
                for batch in inflight.drain(..) {
                    for (_, tx) in batch.callbacks {
                        let _ = tx.send(Err(LyraError::Canceled));
                    }
                }
                for req in batch.drain(..) {
                    let _ = req.reply.send(Err(LyraError::Canceled));
                }
                return;
            }
